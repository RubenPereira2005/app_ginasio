...
class PaginaAulas:
    def __init__(self, frame_principal):
        ...
        self.diretorio_base = os.path.dirname(os.path.abspath(__file__))  # Diretório base onde o script está
        self.aulas = self.localizar_imagens(self.diretorio_base)

    def localizar_imagens(self, diretorio):
        tipos_imagem = ('.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp')  # Tipos de arquivos de imagem
        imagens_encontradas = []
        ...
        for root, dirs, files in os.walk(diretorio):
            for file in files:
                if file.lower().endswith(tipos_imagem):
                    ...
                    nome_aula = os.path.splitext(file)[0]  # Remove a extensão do arquivo
                    imagens_encontradas.append({"nome": nome_aula, "imagem": caminho_imagem})
    ...
    def criar_imagem_com_texto(self, imagem, texto, radius=15, border_width=5, border_color="black"):
        ...
        # Criar máscara arredondada para a imagem
        ...
        # Adicionar sobreposição com opacidade à borda
    ...
    def pagina_aulas(self):
        ...
        # Organizar as aulas em linhas de duas
        for i, aula in enumerate(self.aulas):
            ...
            try:
                ...
                img = img.resize((150, 150), Image.Resampling.LANCZOS)  # Imagens menores (150x150)
                img = self.criar_imagem_com_texto(img, aula["nome"])  # Borda arredondada e texto
            ...
            label_aula.image = img_tk  # Referência para a imagem
            label_aula.pack()

        # Ajuste para a última aula ficar centralizada se houver 5 aulas
        if len(self.aulas) == 5:
            frame_aulas.grid_columnconfigure(1, weight=1)  # Centraliza a última aula na segunda coluna

        # Atualizar a área de visualização da barra de rolagem
